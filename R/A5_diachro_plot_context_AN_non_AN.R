...
AN_C_mean <- mean(AN_C$Tx_POS_EnonE, na.rm=TRUE)
  AN_C_mean <- round(AN_C_mean, digits=2)
AN_C_median <- median(AN_C$Tx_POS_EnonE, na.rm=TRUE)
  AN_C_median <- round(AN_C_median, digits=2)
AN_C_sd <- sd(AN_C$Tx_POS_EnonE, na.rm=TRUE)
  AN_C_sd <- round(AN_C_sd, digits=2)
AN_C_sum <- sum(AN_C$Nb_POS_EnonE, na.rm=TRUE)
  AN_C_sum <- round(AN_C_sum, digits=2)
AN_C_date_mean <- mean(AN_C$datecomposition, na.rm=TRUE)
  AN_C_date_mean <- round(AN_C_date_mean, digits=0)
AN_C_corr <- wtd.cor(AN_C$datecomposition, AN_C$Tx_POS_EnonE)
 AN_C_corr <- as.data.frame(AN_C_corr)
  AN_C_rho <- AN_C_corr$correlation
		 AN_C_rho <- round(AN_C_rho, digits=3)
		AN_C_SE <- AN_C_corr$std.err
		 AN_C_SE <- round(AN_C_SE, digits=3)
		AN_C_tvalue <- AN_C_corr$t.value
			AN_C_tvalue <- round(AN_C_tvalue, digits=3)
		AN_C_pvalue <- AN_C_corr$p.value
			AN_C_pvalue <- round(AN_C_pvalue, digits=5)

p_AN_C <- ggplot(AN_C, aes(datecomposition, Tx_POS_EnonE)) +
	geom_point() +
...
AN_V_mean <- mean(AN_V$Tx_POS_EnonE, na.rm=TRUE)
  AN_V_mean <- round(AN_V_mean, digits=2)
AN_V_median <- median(AN_V$Tx_POS_EnonE, na.rm=TRUE)
  AN_V_median <- round(AN_V_median, digits=2)
AN_V_sd <- sd(AN_V$Tx_POS_EnonE, na.rm=TRUE)
  AN_V_sd <- round(AN_V_sd, digits=2)
AN_V_sum <- sum(AN_V$Nb_POS_EnonE, na.rm=TRUE)
  AN_V_sum <- round(AN_V_sum, digits=2)
AN_V_date_mean <- mean(AN_V$datecomposition, na.rm=TRUE)
  AN_V_date_mean <- round(AN_V_date_mean, digits=0)
AN_V_corr <- wtd.cor(AN_V$datecomposition, AN_V$Tx_POS_EnonE)
 AN_V_corr <- as.data.frame(AN_V_corr)
  AN_V_rho <- AN_V_corr$correlation
		 AN_V_rho <- round(AN_V_rho, digits=3)
		AN_V_SE <- AN_V_corr$std.err
		 AN_V_SE <- round(AN_V_SE, digits=3)
		AN_V_tvalue <- AN_V_corr$t.value
			AN_V_tvalue <- round(AN_V_tvalue, digits=3)
		AN_V_pvalue <- AN_V_corr$p.value
			AN_V_pvalue <- round(AN_V_pvalue, digits=5)

p_AN_V <- ggplot(AN_V, aes(x=datecomposition, y=Tx_POS_EnonE)) +
	geom_point() +
	geom_smooth(method = "loess",
													se = FALSE,
													na.rm = TRUE,
...
non_AN_C_mean <- mean(non_AN_C$Tx_POS_EnonE, na.rm=TRUE)
  non_AN_C_mean <- round(non_AN_C_mean, digits=2)
non_AN_C_median <- median(non_AN_C$Tx_POS_EnonE, na.rm=TRUE)
  non_AN_C_median <- round(non_AN_C_median, digits=2)
non_AN_C_sd <- sd(non_AN_C$Tx_POS_EnonE, na.rm=TRUE)
  non_AN_C_sd <- round(non_AN_C_sd, digits=2)
non_AN_C_sum <- sum(non_AN_C$Nb_POS_EnonE, na.rm=TRUE)
  non_AN_C_sum <- round(non_AN_C_sum, digits=2)
non_AN_C_date_mean <- mean(non_AN_C$datecomposition, na.rm=TRUE)
  non_AN_C_date_mean <- round(non_AN_C_date_mean, digits=0)
non_AN_C_corr <- wtd.cor(non_AN_C$datecomposition, non_AN_C$Tx_POS_EnonE)
 non_AN_C_corr <- as.data.frame(non_AN_C_corr)
  non_AN_C_rho <- non_AN_C_corr$correlation
		 non_AN_C_rho <- round(non_AN_C_rho, digits=3)
		non_AN_C_SE <- non_AN_C_corr$std.err
		 non_AN_C_SE <- round(non_AN_C_SE, digits=3)
		non_AN_C_tvalue <- non_AN_C_corr$t.value
			non_AN_C_tvalue <- round(non_AN_C_tvalue, digits=3)
		non_AN_C_pvalue <- non_AN_C_corr$p.value
			non_AN_C_pvalue <- round(non_AN_C_pvalue, digits=5)

p_non_AN_C <- ggplot(non_AN_C, aes(datecomposition, Tx_POS_EnonE)) +
	geom_point() +
	geom_smooth(method = "loess",
													se = FALSE,
													na.rm = TRUE,
 ...
 non_AN_C_w_corr <- as.data.frame(non_AN_C_w_corr)
  non_AN_C_w_rho <- non_AN_C_w_corr$correlation
		...
		non_AN_C_w_pvalue <- non_AN_C_w_corr$p.value
			non_AN_C_w_pvalue <- round(non_AN_C_w_pvalue, digits=5)
...
#-------
# non_AN_V: Anglo-norman texts before V-initial word
#-------
...
non_AN_V_mean <- mean(non_AN_V$Tx_POS_EnonE, na.rm=TRUE)
  non_AN_V_mean <- round(non_AN_V_mean, digits=2)
non_AN_V_median <- median(non_AN_V$Tx_POS_EnonE, na.rm=TRUE)
  non_AN_V_median <- round(non_AN_V_median, digits=2)
non_AN_V_sd <- sd(non_AN_V$Tx_POS_EnonE, na.rm=TRUE)
  non_AN_V_sd <- round(non_AN_V_sd, digits=2)
non_AN_V_sum <- sum(non_AN_V$Nb_POS_EnonE, na.rm=TRUE)
  non_AN_V_sum <- round(non_AN_V_sum, digits=2)
non_AN_V_date_mean <- mean(non_AN_V$datecomposition, na.rm=TRUE)
  non_AN_V_date_mean <- round(non_AN_V_date_mean, digits=0)
non_AN_V_corr <- wtd.cor(non_AN_V$datecomposition, non_AN_V$Tx_POS_EnonE)
 non_AN_V_corr <- as.data.frame(non_AN_V_corr)
  non_AN_V_rho <- non_AN_V_corr$correlation
		 non_AN_V_rho <- round(non_AN_V_rho, digits=3)
		non_AN_V_SE <- non_AN_V_corr$std.err
		 non_AN_V_SE <- round(non_AN_V_SE, digits=3)
		non_AN_V_tvalue <- non_AN_V_corr$t.value
			non_AN_V_tvalue <- round(non_AN_V_tvalue, digits=3)
		non_AN_V_pvalue <- non_AN_V_corr$p.value
			non_AN_V_pvalue <- round(non_AN_V_pvalue, digits=5)

p_non_AN_V <- ggplot(non_AN_V, aes(x=datecomposition, y=Tx_POS_EnonE)) +
	geom_point() +
	geom_smooth(method = "loess",
													se = FALSE,
													na.rm = TRUE,
